...
public class RotX {
    ...
    public static String xifraRotX(String msg, int codificacio) {
    ...
    public static String desxifraRotX(String msg, int codificacio) {
        if(codificacio>LOW.length) return "No es una codificacion valida";
        // creamos el string que tendremos que devolver
        StringBuilder resultado = new StringBuilder();
        // analizamos todos los caracteres del texot
    ...
    // Este metodo a partir de un numero que reciba cifrara el texto
    public static void forçaBrutaRotX(String msg) {
    ...
    public static boolean dentroAlfabeto(char caracter, char[] alfabeto) {
    ...
    public static char xifraCaracter(int pos, int mov, char[] alfabeto) {
        int posFinal = pos + mov;
        if(posFinal>=alfabeto.length) posFinal-=alfabeto.length;
        return alfabeto[posFinal];
    }

    public static char desxifraCaracter(int pos, int mov, char[] alfabeto) {
        int posFinal = pos - mov;
        if(posFinal<0) posFinal+=alfabeto.length;
        return alfabeto[posFinal];
    }

    public static int trobaPos(char caracter, char[] alfabeto) {
        for(int i=0; i<alfabeto.length; i++) {
